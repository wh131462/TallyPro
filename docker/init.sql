...
CREATE TABLE IF NOT EXISTS `workshops` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `owner_id` INT UNSIGNED NOT NULL COMMENT '工坊主用户ID',
  `name` VARCHAR(100) NOT NULL COMMENT '工坊名称',
  `description` VARCHAR(500) NOT NULL DEFAULT '' COMMENT '工坊描述',
  `invite_code` VARCHAR(32) NOT NULL DEFAULT '' COMMENT '邀请码',
  `invite_expires_at` DATETIME DEFAULT NULL COMMENT '邀请码过期时间',
  `status` ENUM('active','inactive') NOT NULL DEFAULT 'active' COMMENT '工坊状态',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='工坊表';

-- -----------------------------------------------------------
-- 3. workshop_members 工坊成员表
-- -----------------------------------------------------------
CREATE TABLE IF NOT EXISTS `workshop_members` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `workshop_id` INT UNSIGNED NOT NULL COMMENT '工坊ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='工坊成员表';
...
CREATE TABLE IF NOT EXISTS `skus` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `workshop_id` INT UNSIGNED NOT NULL COMMENT '所属工坊ID',
...
CREATE TABLE IF NOT EXISTS `settlements` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `workshop_id` INT UNSIGNED NOT NULL COMMENT '工坊ID',
  `worker_id` INT UNSIGNED NOT NULL COMMENT '工人用户ID',
