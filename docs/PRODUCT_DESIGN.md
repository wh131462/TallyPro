...
企业主（雇主/工厂主）需要管理旗下员工的计件工作记录。目前可能依赖纸质记录或 Excel，容易出错且不便统计。本小程序旨在将计件记工数字化：企业主设置产品（SKU）和工序，员工每天填报工作量，企业主审核确认，最终按单价自动结算工资。
...
| 角色 | 说明 |
|------|------|
| 企业主（管理员） | 注册后创建"企业"，管理 SKU/工序/员工，审核记录，结算工资 |
| 员工（用户） | 绑定企业主，按天填报工作量 |

> 一个微信号可以同时是企业主和员工（在不同企业下）
...
- 微信授权登录（获取 openid）
- 绑定手机号（微信快速验证）
- 角色切换：同一账号可在"我的企业（管理）"和"我的工作（员工）"间切换
- 个人信息管理
...
- 创建企业（名称、简介）
- 支持创建多个企业（如不同车间/产品线）
- 生成邀请码/二维码供员工扫码绑定（邀请码支持设置有效期，过期自动失效）

#### 3.2.2 员工管理
- 查看绑定申请，审批通过/拒绝
- 主动添加员工（通过手机号，员工登录后自动绑定）
- 编辑员工信息（姓名、备注）
- 移除员工
...
#### 3.2.6 结算管理
- 企业主主动发起结算：选择日期范围 → 生成结算单（草稿）→ 确认结算
- 按月汇总 / 自定义日期范围
- 按员工维度：每个员工的总工量、总金额明细
...
#### 3.3.1 绑定企业主
- 扫码绑定 / 输入邀请码
- 等待企业主审批（企业主也可预先添加手机号，则自动绑定）

#### 3.3.2 每日记工
- 选择日期（默认今天，可选历史日期）
- 查看所有 SKU 列表 → 点击对应工序 → 填写数量
- 支持批量填报：一次性填写多个工序的数量，提高效率
- 常用工序置顶：根据员工历史填报频率，自动将常用工序排在前面
- 已填写的记录可修改（在企业主确认前）
- 查看历史记录及确认状态
...
### 4.3 workshop_members（企业成员表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | INT, PK, AUTO_INCREMENT | 主键 |
| workshop_id | INT, FK→workshops | 企业ID |
...
> **唯一约束**：`UNIQUE(worker_id, step_id, work_date)` — 防止同一员工同一天同一工序重复提交
...
| workshop_id | INT, FK→workshops | 所属企业ID |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/workshops` | 创建企业 |
| GET | `/api/workshops` | 我的企业列表 |
| GET | `/api/workshops/:id` | 企业详情 |
| PUT | `/api/workshops/:id` | 更新企业 |

### 6.3 成员模块
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/workshops/:id/apply` | 员工申请绑定 |
| POST | `/api/workshops/:id/invite` | 企业主邀请（手机号） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/records` | 员工提交记录（写入当时生效单价快照） |
| POST | `/api/records/batch` | 员工批量提交记录（多个工序一次提交） |
| PUT | `/api/records/:id` | 员工修改记录（需校验 version 乐观锁） |
| GET | `/api/records` | 查询记录（支持按日期/员工/SKU 筛选） |
| PUT | `/api/records/:id/confirm` | 企业主确认（需校验 version 乐观锁） |
| PUT | `/api/records/:id/modify` | 企业主修改数量（记录操作日志） |
| POST | `/api/records/batch-confirm` | 批量确认 |

### 6.6 结算模块
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/settlements` | 创建结算单（选择日期范围和员工，生成草稿） |
...
| GET | `/api/settlements/detail` | 明细查询（按员工/SKU） |
...
1. **绑定流程**：员工扫码申请 → 企业主审批 → 绑定成功；或企业主预添加手机号 → 该手机号用户登录后自动绑定
2. **记录修改**：员工可在"待确认"状态下修改；企业主确认后员工不可改，仅企业主可改；已结算记录不可修改
3. **工序可见性**：员工可看到企业主企业的所有 SKU 和所有工序，自行选择填报
4. **单价快照**：员工提交记录时，系统自动写入当时生效的单价到记录中，后续调价不影响已提交的记录
5. **单价变更**：调价后新记录用新价，已提交的记录保持原价（通过 unit_price 快照 + price_history 双重保障）
6. **结算计算**：总金额 = Σ(各记录的确认数量 × 该记录的单价快照)
7. **结算流程**：企业主选择日期范围 → 系统生成结算单草稿 → 企业主确认 → 相关记录标记为已结算 → 员工收到通知
8. **数据安全**：员工只能看到自己的数据，企业主只能看到自己企业的数据
9. **多企业支持**：一个企业主可创建多个企业（如不同车间/产品线），员工可绑定同一企业主的不同企业
10. **并发控制**：通过 version 乐观锁防止企业主和员工同时操作同一条记录导致数据冲突
11. **防重复提交**：同一员工同一天同一工序只能有一条记录（数据库唯一约束）
...
1. 注册两个测试账号，分别扮演企业主和员工
2. 企业主创建企业 → 设置 SKU → 添加工序和单价
3. 员工扫码申请绑定 → 企业主审批通过 → 双方收到通知
4. 员工连续多天填报记录（含批量填报）→ 企业主逐条确认/修改
5. 验证单价快照：提交记录后调价，确认旧记录仍使用旧单价
6. 验证乐观锁：模拟企业主和员工同时操作同一记录，确认冲突被正确拦截
7. 验证防重复提交：同一员工同一天同一工序重复提交应被拒绝
8. 企业主创建结算单 → 确认结算 → 验证记录状态变为已结算 → 员工收到通知查看工资
9. 验证结算明细：核对结算单中每条明细的数量、单价、小计是否正确
10. 验证按月结算金额计算正确性
11. 测试软删除：删除 SKU/工序后，历史记录和结算单仍正常展示
12. 测试邀请码过期：过期邀请码应无法绑定
13. 查看操作日志：确认关键操作（审核、修改、结算）均有完整记录
14. 测试边界情况：跨月记录、同一天多次修改、企业主修改后员工查看、已结算记录不可修改
